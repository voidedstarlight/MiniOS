...
Mini Terminal
Terminal Emulator made using Python 3 and PyQt for the MiniOS project: https://github.com/voidedstarlight/MiniOS
"""
...
class TextEdit(QTextEdit):
	...
	def keyPressEvent(self, event: QKeyEvent) -> None:
		if event.key() in [Qt.Key_Enter, Qt.Key_Return] and self.return_pressed_event is not None:
			...
			return
		if (self.toPlainText().splitlines()[-1] == self.PS1 and event.key() in [Qt.Key_Backspace, Qt.Key_Left, Qt.Key_Right]) or (event.key() == Qt.Key_H and QApplication.keyboardModifiers() == Qt.MetaModifier) or (event.key() == Qt.Key_Left and self.toPlainText()[:self.textCursor().position()].splitlines()[-1] == self.PS1): return
		if event.key() in [Qt.Key_Up, Qt.Key_Down]:
			if (len(open("Applications/MiniTerminal/history.txt", "r").read().splitlines()) < self.history_entry_index + 2 and event.key() == Qt.Key_Up) or (self.history_entry_index - 1 < 0 and event.key() == Qt.Key_Down): return
	...
	def mouseDoubleClickEvent(self, _) -> None: return
