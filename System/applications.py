...
def getApplicationProperties(rel_path: str) -> dict:
	...
	if "name" not in load(open(f"{rel_path}/application.json")) or "run" not in load(open(f"{rel_path}/application.json")) or "run_class" not in load(open(f"{rel_path}/application.json")) or "background-color" not in load(open(f"{rel_path}/application.json")) or "window-size" not in load(open(f"{rel_path}/application.json")): return {"valid": False}
...
def returnApplications() -> dict:
	...
	for i in sorted(__import__("os").listdir("Applications"))[1:]:
		...
		if properties["valid"]: applications[i] = {"name": properties["name"], "path": properties["path"], "file": properties["path"].split("/")[-1], "run_class": properties["run_class"], "background-color": properties["background-color"], "window-size": properties["window-size"]}
	return applications

def returnApplicationProperties() -> dict:
	"""Return properties for each application"""
	applications, properties = returnApplications(), {}
	for i in applications.keys(): properties[applications[i]["name"]] = {x: y for x, y in applications[i].items() if x != "name"}
	return properties
