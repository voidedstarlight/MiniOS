...
class ThemeConfig:
	...
	def formatJSON() -> None:
		with open("System/config/theme.json") as file:
			...
			if open("System/config/theme.json", "r+").read() == "": open("System/config/theme.json", "w").write("{ }")
			if "theme" not in loaded_file: Config.appendJSON({"theme": "light"}, "System/config/theme.json")
			if open("System/config/theme.json", "r+").read().splitlines()[-1] != "": open("System/config/theme.json", "a").write("\n")
