...
class Window(QMainWindow):
	...
	def __init__(self, parent=None) -> None:
		...
		self.clock.setUrl(QUrl(f"https://home.voidedstarlight.repl.co/clock.html?background_color={returnBackgroundProperties()['background-color-2'][1:]}&text_color={returnBackgroundProperties()['text-color'][1:]}&font_size={returnProperties()['font-size']}px&font_family={returnProperties()['font-family']}")) # Set URL for web engine view
		# # Separator
	...
	def openApplication(self, app: str) -> None:
		...
		exec(f"self.windows.append(ApplicationWindow(self, QPoint(0, 0), {app}(), background_color = '{_applications.returnApplicationProperties()[app]['background-color'] if _applications.returnApplicationProperties()[app]['background-color'] != 'default' else returnBackgroundProperties()['background-color']}', window_name = '{app}', toolbar_background_color = '{returnBackgroundProperties()['background-color-3']}', window_size = {_applications.returnApplicationProperties()[app]['window-size']}))") # Create new window with child widget of application class
	...
	def updateElements(self) -> None:
		...
		self.clock.setUrl(QUrl(f"https://home.voidedstarlight.repl.co/clock.html?background_color={returnBackgroundProperties()['background-color-2'][1:]}&text_color={returnBackgroundProperties()['text-color'][1:]}&font_size={returnProperties()['font-size']}px&font_family={returnProperties()['font-family']}")) # Update clock styles
